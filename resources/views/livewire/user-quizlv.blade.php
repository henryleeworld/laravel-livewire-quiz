    ...
    <div class="px-4 -py-3 sm:px-6 ">
        <div class="flex max-w-auto justify-between">
            <h1 class="text-sm leading-6 font-medium text-gray-900">
                <span class="text-gray-400 font-extrabold p-1">{{ __('User') }}</span>
                <span class="font-bold p-2 leading-loose bg-blue-500 text-white rounded-lg">{{Auth::user()->name}}</span>
            </h1>
            <p class="mt-1 max-w-2xl text-sm text-gray-500">
                <span class="text-gray-400 font-extrabold p-1">{{ __('Quiz Progress') }} </span>
        ...
        <div class="bg-white border-2 border-gray-300 shadow overflow-hidden sm:rounded-lg">
            <div class="container px-5 py-5 mx-auto">
                <div class="text-center mb-5 justify-center">
                    ...
                    <p class="text-md mt-10"> {{ __('Dear') }} <span class="font-extrabold text-blue-600 mr-2"> {{Auth::user()->name.'!'}} </span> {{ __('You have secured') }} <a class="bg-green-300 px-2 mx-2 hover:green-400 rounded-lg underline" href="{{route('quiz.show',$quizid) }}">{{ __('Show quiz details') }}</a></p>
                ...
                <div class="flex flex-wrap lg:w-4/5 sm:mx-auto sm:mb-2 -mx-2">
                    ...
                    <div class="p-2 sm:w-1/2 w-full">
                        <div class="bg-gray-100 rounded flex p-4 h-full items-center">
                            ...
                            <span class="title-font font-medium mr-5 text-purple-700">{{ __('Quiz Status') }}</span><span class="title-font font-medium">{{ $quizPecentage > 70 ? __('Pass') : __('Fail') }}</span>
                        </div>
                    </div>
                </div>
                <div class="mx-auto min-w-full p-2 md:flex m-2 justify-between">
                    <a href="{{route('quiz.show',$quizid) }}" class="text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">{{ __('See Quizzes Deatils') }}</a>
                    <a href="{{route('userQuizHome')}}" class="text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">{{ __('See All Your Quizzes') }}</a>
                </div>
            </div>
        </div>
    </section>
                    ...
                    <div class="h-full bg-gray-100 p-8 rounded">
                        ...
                        <a class="inline-flex items-center">
                            <span class="flex-grow flex flex-col">
                                <span class="title-font font-medium text-gray-900">{{ __('Author') }}</span>
